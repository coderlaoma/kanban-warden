...
@pytest.mark.parametrize(
    ("base_branch", "merge_commit_sha", "merged_by", "merged_at"),
    [
        ("", "abc1234", "lead", "2026-06-23T10:00:00Z"),
        ("main", "", "lead", "2026-06-23T10:00:00Z"),
        ("main", "abc1234", "", "2026-06-23T10:00:00Z"),
        ("main", "abc1234", "lead", ""),
    ],
)
def test_self_improvement_merge_requires_merge_identity(
    tmp_path: Path,
    base_branch: str,
...
def test_self_improvement_deployment_plan_must_match_merge_commit(tmp_path: Path) -> None:
    store = WardenStateStore(tmp_path / "state.db")
    draft = _prepare_merged_code_change(store)
...
def test_self_improvement_deployment_plan_requires_monitor_window(tmp_path: Path) -> None:
    store = WardenStateStore(tmp_path / "state.db")
    draft = _prepare_merged_code_change(store)
...
def test_self_improvement_deployment_plan_requires_plugin_version(tmp_path: Path) -> None:
    store = WardenStateStore(tmp_path / "state.db")
    draft = _prepare_merged_code_change(store)
...
def test_self_improvement_rollback_requires_restored_identity(
    tmp_path: Path,
